...
case class WebSocketGraphQLClient(uri: String)(implicit csIO: ContextShift[IO])
    extends GraphQLStreamingClient[ConcurrentEffect] {

  type Subscription[F[_], D] = WebSocketSubscription[F, D]

  case class WebSocketSubscription[F[_]: LiftIO, D](stream: Stream[F, D], private val id: String)
      extends Stoppable[F] {
    def stop: F[Unit] =
  ...
  lazy private val client: Deferred[IO, Either[Exception, WebSocketSender]] = {
    ...
    try {
      ...
      ws.onerror = { e: Event =>
        deferred
          .complete(
            parse(js.JSON.stringify(e)).flatMap(json => Left(new GraphQLException(List(json))))
          )
          .recover {
